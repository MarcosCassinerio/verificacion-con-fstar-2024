...
estos tipos? *)
let incr'    (x:nat) : int = addnat x 1
let incr''   (x:nat) : nat = addnat x 1
let incr'''  (x:nat) : y:int{y = x+1} = addnat x 1
...
let rec fib' (x:nat) : y:pos{y >= x} = if x <= 1 then 1 else (fib' (x - 1)) + (fib' (x - 2))
...
let rec append #a (xs:(list a)) (ys:(list a)) : (list a) = match xs with
                       | [] -> ys
...
let rec rev_int xs = match xs with
                     | [] -> []
                     | hd :: tl -> append (rev_int tl) (Cons hd Nil)
